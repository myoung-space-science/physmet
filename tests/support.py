...
from physmet.numeric import Measurement
...
class Measurable:
    ...
    `~base.Measurable` protocol and called by `~data.measure`. It maintains
    internal attributes for numeric data and a metric unit, but does not define
    any public properties.
    """

    def __init__(self, data, /, unit=None) -> None:
        self._data = data
        self._unit = metric.unit(unit or '1')

    def __measure__(self) -> Measurement:
        return measured.sequence(self._data, unit=self._unit)
...
def compute_unit(f, a, b):
    ...
    if isinstance(a, numbers.Real) and isinstance(b, Measurement):
        return f('1', b.unit)
    if isinstance(a, Measurement) and isinstance(b, numbers.Real):
        return a.unit
    if isinstance(a, tuple) and isinstance(b, Measurement):
        return f(a[-1], b.unit)
    if isinstance(a, Measurement) and isinstance(b, tuple):
        return f(a.unit, b[-1])
    raise TypeError(a, b)
...
def getdata(a):
    ...
    if isinstance(a, (numbers.Number, numpy.number, numpy.ndarray)):
        return a
    exceptions = (
        measurable.ParsingTypeError,
        measurable.ParsingValueError,
    )
    with contextlib.suppress(*exceptions):
        parsed = measurable.parse(a, distribute=False)
        values = parsed[:-1]
        if len(values) > 1:
